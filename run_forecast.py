...
def upload_forecast_to_s3():
    ...
    log(f"Uploaded {OUTPUT_PARQUET} to s3://{bucket_name}/{s3_key}")
...
def main():
    try:
        ...
        log(f"Forecasting window: {forecast_start.date()} to {forecast_end.date()}")
        ...
        log(f"Saved clean forecast to {OUTPUT_PARQUET}")
